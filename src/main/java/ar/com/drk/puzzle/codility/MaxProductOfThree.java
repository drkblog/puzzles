...
public class MaxProductOfThree {
  ...
  public static void main(final String[] args) {
    final int[] A = new int[]{-3, 1, 2, -2, 5, 6};
    final int[] B = new int[]{-5, 5, -5, 4};
    System.out.println("Max triplet is: " + solution(A));
  }

  static int solution(final int[] A) {
    final Heap maxHeap = Heap.getMaxHeap(3);
    final Heap minHeap = Heap.getMinHeap(2);
  ...
  static class Heap {
    ...
    private static class HighestValues implements HeapStrategy {
      ...
      @Override
      public int findLimitPosition(final int[] heap, final int count) {
      ...
      @Override
      public boolean shouldReplace(final int value, final int limit) {
        return value > limit;
      }
    }
    ...
    public static Heap getMaxHeap(final int size) {
      return new Heap(size, new HighestValues());
    }

    public static Heap getMinHeap(final int size) {
